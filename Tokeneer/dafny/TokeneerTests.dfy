...
module TokeneerTests {
  ...
  class IdStationTests {
    ...
    static method {:synthesize "fresh"} fresh_IdStation() returns (idStation : IdStation)
      ensures fresh(idStation)

    static method {:synthesize "mock"} mock_Token_OpenVersion0_NotIsValid() returns (token : Token) 
      ensures fresh(token)
      ensures forall fingerprint : int :: token.fIsValid(fingerprint) == false;
      ensures token.getClearanceLevel() == Confidential

    static method {:synthesize "mock"} mock_Token_OpenVersion0_IsValid_NotHasClearance() returns (token : Token) 
      ensures fresh(token)
      ensures forall fingerprint : int :: token.fIsValid(fingerprint) == true;
      ensures token.getClearanceLevel() == Confidential

    static method {:synthesize "mock"} mock_Token_OpenVersion0_IsValid_HasClearance() returns (token : Token) 
      ensures fresh(token)
      ensures forall fingerprint : int :: token.fIsValid(fingerprint) == true;
